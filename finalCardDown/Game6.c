...
int gameWinner(Game game)
{
	...
	// discardPile, the game ends with no winner
	if (getDeckCard(game, 0) == NULL && getDiscardPileCard(game, 0) == 0)
	{
		winner = NO_WINNER;
	}
...
int isSpecialCard(Card card)
{
	return (cardValue(card) == D || cardValue(card) == A || cardValue(card) == DRAW_TWO);
}
