 ...
 *
 *
 * E: Return values from each function
 *
 * F: Use someone else's testGame.c to add tests
 */
...
static int checkCard(Card car, value v, color c, suit s, int curr, value values[], color colors[], suit suits[]);
...
static void checkReturnValues();
static void playTwoRounds();
...
static void playTwoRounds()
{
    // Use a random card finder to check what happens
    // during any random normal cases
    // (The deck is not randomizes)
    printf("\n---------------------------------------------\n");
    printf("            Other Tests\n");
    printf("---------------------------------------------\n");

    // Other cases
    // Check return values of functions not called
    // in the functions above
    // while playing two rounds of the game
    checkReturnValues();
...
static void isValidMoveCheckPlayer3(Game game)
{
	...
	while (isValidMove(game, move))
	{
		...
		printCardByComponents(cardValue(drawn), cardColor(drawn), cardSuit(drawn));
		printf("\n");
		hcard = findCardInHand(game, 3, cardValue(drawn), cardColor(drawn), cardSuit(drawn));
		assert(hcard != NULL);
...
static int checkCard(Card car, value v, color c, suit s, int curr, value values[], color colors[], suit suits[])
{
    if (car != NULL)
    {
        ...
        return (cardValue(car) == v && cardColor(car) == c && cardSuit(car) == s);
    }
    else
    {
        return -1;
...
// Finds a card from the hand that can be played
